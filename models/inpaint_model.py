...
from . import inpaint_network


class inpaint_model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = inpaint_network.define_G(netG_input_nc, opt.output_nc, opt.ngf, opt.netG,
        ...
        if self.isTrain:
            ...
            self.criterionGAN = inpaint_network.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor)
            self.criterionFeat = torch.nn.L1Loss()
            if not opt.no_vgg_loss:
                self.criterionVGG = inpaint_network.VGGLoss(self.gpu_ids)
    ...
    def forward(self, img_agnostic, pose, warped_c,  parse_div, misalign_mask, ground_truth_image, infer=False):
        ...
        pred_fake_pool = self.discriminate(torch.cat((img_agnostic,parse_div,pose), dim=1), fake_image, use_pool=True)#discriminator는 좀 더 어려운걸 학습해야
        loss_D_fake = self.criterionGAN(pred_fake_pool, False)

        # Real Detection and Loss
        pred_real = self.discriminate(torch.cat((img_agnostic,parse_div,pose), dim=1), ground_truth_image)
        loss_D_real = self.criterionGAN(pred_real, True)

        # GAN loss (Fake Passability Loss)
        pred_fake = self.netD.forward(torch.cat((torch.cat((img_agnostic,parse_div,pose), dim=1), fake_image), dim=1))
    ...
    def inference(self, img_agnostic, pose, warped_c, parse, parse_div, misalign_mask, agnostic_mask):
        ...
        with torch.no_grad():
            fake_image = self.netG.forward(torch.cat((img_agnostic, pose, warped_c), dim=1), parse, parse_div,
                                           misalign_mask)
...
class InferenceModel(inpaint_model):
